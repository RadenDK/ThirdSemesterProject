...
namespace GameClientApiTests.BusinessLogicTests
{
    ...
    WHERE PlayerId IN (1, 2, 4)";
        ...
        [Fact]
        public void CreateGameLobby_TC1_RollsBackTransaction_WhenErrorOccurs()
        {
            // Arrange
            var mockGameLobbyDatabaseAccessor = new Mock<IGameLobbyDatabaseAccessor>();
            var mockPlayerDatabaseAccessor = new Mock<IPlayerDatabaseAccessor>();
            PlayerLogic playerLogic = new PlayerLogic(_configuration, mockPlayerDatabaseAccessor.Object);
            IGameLobbyLogic SUT = new GameLobbyLogic(_configuration, mockGameLobbyDatabaseAccessor.Object, playerLogic);

            GameLobbyModel newLobby = new GameLobbyModel
            {
                LobbyName = "TestLobby",
                AmountOfPlayers = 5,
                InviteLink = "TestLink",
            ...
            // Setup mock to throw an exception when CreateGameLobby is called
            mockGameLobbyDatabaseAccessor.Setup(m => m.CreateGameLobby(It.IsAny<GameLobbyModel>(), It.IsAny<SqlTransaction>()))
                .Throws(new Exception("Error"));

            // Act
            try
            {
                SUT.CreateGameLobby(newLobby, username);
            }
            catch
            {
                // Ignore the exception for the purposes of this test
            }

            // Assert
            mockGameLobbyDatabaseAccessor.Verify(m => m.RollbackTransaction(It.IsAny<SqlTransaction>()), Times.Once);
        }
