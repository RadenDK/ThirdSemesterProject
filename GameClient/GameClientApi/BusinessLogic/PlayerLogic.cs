...
namespace GameClientApi.BusinessLogic
{
    public class PlayerLogic : IPlayerLogic
    {
        ...
        private bool AccountHasValues(AccountRegistrationModel newAccount)
        {
            ...
            if (newAccount.BirthDay == null || newAccount.BirthDay < new DateTime(1753, 1, 1) ||
                newAccount.BirthDay > new DateTime(9999, 12, 31))
            {
                return false;
            }
        ...
        public bool Logout(int playerId, SqlTransaction transaction = null)
        {
            PlayerModel player = new PlayerModel { PlayerId = playerId, IsOwner = false, OnlineStatus = false };
            try
            {
                transaction = _playerAccessor.BeginTransaction();
                GameLobbyModel emptyGameLobbyModel = new GameLobbyModel { GameLobbyId = null };
                UpdatePlayerLobbyId(player, emptyGameLobbyModel, transaction);
            ...
            {
                _playerAccessor.RollbackTransaction(transaction);
                throw new Exception("Failed to logout your player: " + ex.Message);
            }
        }
    }
}